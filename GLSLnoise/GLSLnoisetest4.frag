...
void main( void )
{
  ...
  //float n = noise(vec3(0.04 * v_texCoord3D.xyz * (2.0 + sin(time))));
  //float n = snoise(vec3(0.04 * v_texCoord3D.xyz * (2.0 + sin(time))));
  //float n = noise(vec4(0.08 * v_texCoord3D.xyz, time));
  float n = snoise(vec4(0.04 * v_texCoord3D.xyz, time));
