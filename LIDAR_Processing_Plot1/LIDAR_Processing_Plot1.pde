...
final boolean DUMPPACKETS = true;
final boolean DUMPHEX = true;
final boolean SHOWPROGRESS = false;
final boolean SHOWPOINTSPLOTTED = false;
final int CLEARCOUNT = 4000;   // clear the screen this often (points plotted)
int pointsPlotted = 0;
final boolean DRAWRETICLE = true;
...
void draw() { 
  ...
  //if (frameCount % CLEARCOUNT == 0) {
  if (pointsPlotted >= CLEARCOUNT) {
    background(204);
    if (SHOWPOINTSPLOTTED){
      print("pointsPlotted="); println(pointsPlotted);
    }
    pointsPlotted = 0;
  }
  
  if (DRAWRETICLE) DrawReticle();  
  PlotPacket(p);
