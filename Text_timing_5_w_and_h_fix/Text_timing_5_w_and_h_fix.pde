...
void draw() {
  ...
  float maxSizeH = fontSize/(textDescent()+textAscent()) * (height-100);   // maximum font size to fit in the frame height-wise with a bit of space
  ...
  // Draw Text
  text(words[index], width/2, (height/2));  // Places text in middle of window.
