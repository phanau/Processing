...
class ImageSource implements Source {   // Implements the interface, Source (see above)
  ...
  public ImageSource(String path) {  // "Constructor" of class image source
    ...
    if (mbFilter)
      myFilter(mImage, mFilter, mParam);
  }
  ...
  // Same four functions as up above for interface being implemented for a particular source
...
class CameraSource implements Source {
  ...
  public PImage get() {
    if (available()) {
      ...
      mFrame = mCamera.get();
      if (mbFilter)
        myFilter(mFrame, mFilter, mParam);
    }
...
void setup() {    // Customize this and the draw loop by adding my own media
  ...
  layers[indx++] = new SourceLayer(new CameraSource(new Capture(this, width, height)).filter(ERODE,8)).flip(true,false); // flip(horizontal,vertical)
  ...
  // a Layer that shows a Movie masked by the camera
  //layers[indx++] = new SourceLayer(new MovieSource(new Movie(this, "RiverCloseup.mp4"))).mask(new CameraSource(new Capture(this, width, height)).filter(THRESHOLD,0.5)).flip(true,false);
  //layers[indx++] = new SourceLayer(new MovieSource(new Movie(this, "RiverCloseup.mp4"))).mask(new CameraSource(new Capture(this, width, height)).filter(POSTERIZE,4)).flip(true,false);
