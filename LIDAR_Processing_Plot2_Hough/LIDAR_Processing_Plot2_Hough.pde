...
// keep track of how much of the "circle" has data points
...
// compute how much of the "circle" has data points
void Update(Packet p) {
  for (int i=(int)p.mStartAngle; i<=(int)p.mEndAngle; i++) {
    if (mCoverage[i]==0)
      mPctCoverage += 1.0/360.;
    mCoverage[i]++;
  }
}

void Clear() {
  mCoverage = new int[360];
  mPctCoverage = 0.0;
}
...
void setup() {
  ...
  // create object for keeping track of data coverage
