
import processing.serial.*;
 
Serial myPort;

void setup() {
  // List all the available serial ports
  printArray(Serial.list());
  // Open the port you are using at the rate you want:
  myPort = new Serial(this, Serial.list()[7], 230400);
  
  // process some packets
  for (int i=0; i<4; i++)
    getPacket();
    
  exit();
}

void getPacket() {
  println();println("*****************************");
  
  // discard data while waiting for start of packet sync character
  boolean sync = false;
  do {
    while (myPort.available() < 1);  // wait for valid data
    sync = (myPort.read() == 0x54);  // test for sync byte
  } while (!sync);

  // get packet data length = number of data points
  while (myPort.available() < 1);
  int numPoints = myPort.read() & 0x1F;  // 5 bits
  int packLen = numPoints*3 + 9;
  print ("packet length: "); println(packLen);

  // wait for packet data
  while (myPort.available() < packLen);
   
  // get Radar Speed
  int radarSpeed = myPort.read() + 256*myPort.read();
  print ("Radar Speed: "); println(radarSpeed);

  // get Start Angle
  int startAngle = myPort.read() + 256*myPort.read();
  print ("Start Angle: "); println(startAngle/100.0);
  
  print ("numPoints: "); println(numPoints);
  
  // get the data points
  for (int i=0; i<numPoints; i++) {
    int dist = myPort.read() + 256*myPort.read();
    int conf = myPort.read();
    print("distance: "); print(dist);
    print("  confidence: "); println(conf);
  }
  
  // get End Angle
  int endAngle = myPort.read() + 256*myPort.read();
  print ("End Angle: "); println(endAngle/100.0);

  // get Timestamp
  int timestamp = myPort.read() + 256*myPort.read();
  print ("Timestamp: "); println(timestamp);
 
  // get checksum
  int chksum = myPort.read();
  print ("checksum: "); println(chksum);
}

/*
private final static uint8_t[] CrcTable = {

}

uint8_t CalCRC8(uint8_t *p, uint8_t len) {
  uint8_t crc = 0; uint16_t i;
  for (i = 0; i < len; i++) {
    crc = CrcTable[(crc ^ *p++) & 0xff]; 
  }
  return crc; 
}
*/



