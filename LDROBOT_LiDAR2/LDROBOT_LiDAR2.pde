...
Packet GetPacket() {
  ...
  // discard data while waiting for start of packet sync character
  boolean sync = false;
  do {
  ...
  // get packet data length = number of data points
  while (myPort.available() < 1);
  int numPoints = myPort.read() & (byte)0x1F;  // 5 bits
  int packetLen = numPoints*3 + 9;
  
  // wait for packet data
  while (myPort.available() < packetLen);
   
  // get Radar Speed
  p.mSpeed = myPort.read() + 256*myPort.read();

  // get Start Angle
  p.mStartAngle = myPort.read() + 256*myPort.read();
  ...
  for (int i=0; i<numPoints; i++) {
    p.mDistances[i] = myPort.read() + 256*myPort.read();
    p.mConfidences[i] = myPort.read();
  }
  
  // get End Angle
  p.mEndAngle = myPort.read() + 256*myPort.read();
 
  // get checksum
  p.mChecksum = myPort.read();
...
void DumpPacket(Packet p) {
  ...
  print ("Start Angle: "); println(p.mStartAngle/100.0);
  print ("End Angle: "); println(p.mEndAngle/100.0);
  ...
  for (int i=0; i<numPoints; i++) {
    ...
    if(p.mEndAngle > p.mStartAngle)
      ...
      step = (p.mEndAngle + 36000 - p.mStartAngle)/(numPoints - 1);
    int angle = p.mStartAngle + step*i;
    print("  angle: "); print(angle/100.0);
    print("  confidence: "); println(p.mConfidences[i]);
  }
  }
  
  print ("checksum: "); println(p.mChecksum);
}
...
static byte CalCRC8(byte[] p) {
  byte crc = 0;
  for (int i = 0; i < p.length; i++) {
    crc = CrcTable[(crc ^ p[i]) & (byte)0xff]; 
  }
  return crc; 
}
