...
void setup() {    // Customize this and the draw loop by adding my own media
  //size(1280, 800);
  size(764,564);  // same size as MLK movie below
  frameRate(30); //<>//
  colorMode(RGB, 1.0);
  ...
  // a Layer that shows a Movie masked by the camera that has been run through one or more filters
  //layers[indx++] = new SourceLayer(new MovieSource(new Movie(this, "RiverCloseup.mp4"))).mask(new CameraSource(new Capture(this, width, height)).filter(ERODE, 4).filter(THRESHOLD,0.5)).flip(true,false);
  ...
  layers[indx++] = new SourceLayer(new MovieSource(new Movie(this, "MLK_speech.mov")));
  //layers[indx++] = new SourceLayer(new MovieSource(new Movie(this, "MLK_speech.mov")).filter(INVERT));
  layers[indx++] = new SourceLayer(new MovieSource(new Movie(this, "MLK_text.mov"))).mask(new MovieSource(new Movie(this, "MLK_text.mov")).filter(INVERT));
...
void draw() {
  ...
  if (keyPressed && key=='s')
    saveFrame("saveFrame"+guid+"/frame-#####.jpg");
}