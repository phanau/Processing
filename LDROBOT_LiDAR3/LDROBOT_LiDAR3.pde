...
// if we try to process everything we get way behind, so we clear the input buffer
// after processing each packet and then try to re-sync to the stream next time.
// this works for a while but eventually stalls for reasons TBD.
...
// this works when things are proceeding normally, but not after things lock up :(
void keyPressed() {
  println("\nkey pressed = RESTART");
  // Close and then reopen the port you are using at the rate you want:
  myPort.stop();
  myPort = new Serial(this, Serial.list()[7], 230400);
}
...
void draw() {  
  ...
  // fade previously plotted points over time
  fill(0x02FFFFFF);
  stroke(0x02FFFFFF);
  ...
  // make sure we don't get too far behind ... this works for a while but eventually causes a stall
  
  final int BEHIND = 500;
  if (myPort.available() > BEHIND) {
    //myPort.clear();    // this stalls pretty quickly

    // try this -- read a fixed number of bytes
    byte[] inBuffer = new byte[BEHIND];
    int count = myPort.readBytes(inBuffer);
    print(" discard = "); print(count); print("  available = "); print(myPort.available()); println();
    //print("x");
  }
...
Packet GetPacket() {
  ...
  while (myPort.available() < packetLen);
  ...
  // get Radar Speed
  p.mSpeed = (data[2] + 256*data[3])&0x7FFF;

  // get Start Angle
  p.mStartAngle = ((data[4] + 256*data[5])&0x7FFF)/100.0;
  ...
  for (int i=0; i<numPoints; i++) {
    p.mDistances[i] = (data[3*i+6] + 256*data[3*i+7])&0x7FFF;
    //p.mDistances[i] = (int)p.mStartAngle*5; // uncomment this to test graphics
    p.mConfidences[i] = data[3*i+8]&0xFF;
  }
  
  // get End Angle
  p.mEndAngle = ((data[3*numPoints+6] + 256*data[3*numPoints+7])&0x7FFF)/100.0;
 
  // get Timestamp
  p.mTimestamp = (data[3*numPoints+8] + 256*data[3*numPoints+9])&0x7FFF;
...
static color[] colors = { #FF0000, #00FF00, #0000FF, #000000 };
...
void PlotPacket(Packet p) {
  ...
  for (int i=0; i<p.mDistances.length; i++){
    int x = (int)(p.mDistances[i]*sin(GetAngle(p,i)*TWO_PI/360.0));
    int y = (int)(-p.mDistances[i]*cos(GetAngle(p,i)*TWO_PI/360.0));
    if (p.mConfidences[i] > 100)
      point(x,y);
  }
}
